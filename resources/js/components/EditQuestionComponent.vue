            ...
            <div class="col-md-12">
                <ul class="text-danger" v-if="isError">
                    <li v-for="(error, index) in errors" :key="index">
                        <span v-for="(errr, index) in error" :key="index"
                            >{{ errr }}
                        </span>
                    </li>
                </ul>
                ...
                <div class="alert alert-success" v-if="status">
                    <div class="alert-header" @click="toggleStatus">
                        &times;
                    </div>
                    Question Updated
                </div>
                ...
                <div class="my-5 p-3 card">
                    ...
                    >
                        ...
                        <option
                            ...
                            :value="group.id"
                            :key="group.id"
                            v-if="group.title !== 'Select'"
                        >
                            {{ group.id }} - {{ group.title }}
                        </option>
                    </select>
                    <div class="d-flex justify-content-end">
                        <button @click="showAddGroupTitle = true" class="btn">
                            Add New
                        </button>
                    </div>
                        ...
                        <div class="form-group">
                            <div class="bg-light w-100 p-3">
                                ...
                                <input
                                    ...
                                    placeholder="Enter group info/title"
                                />
                                <div class="my-2">
    ...
    methods: {
        ...
        async getCategoriesByTitle(title) {
            let response = await axios.get(
                this.appURL + `/get-categories-bytitle/${title}`
            );
        ...
        async addQuestionGroup() {
            if (this.group_title == "" || this.group_category == "")
                return false;
            var data = {
            ...
            let response = await axios.post(
                this.appURL + "/add-question-group",
                data
            );
        ...
        async updateQuestion() {
            ...
            await axios
                .post(
                    this.appURL +
                        `/admin/questions/update/${this.thisquestion.id}`,
                    data
                )
                .then((response) => {
                    this.isError = false;
        ...
        async getQuestion() {
            let response = await axios.get(
                this.appURL + `/get-question-byid/${this.thisquestion.id}`
            );
