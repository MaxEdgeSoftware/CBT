...
Route::domain('{subdomain}.' . env('APP_CDN'))->group(function ($subdomain) {
    Route::get('/', function ($subdomain) {
        $school = School::where("domain", $_SERVER["HTTP_HOST"])->firstOrFail();
        $landing = Landing::where("school_id", $school->id)->first();
        return view('main.home', ['landing' => $landing, "school" => $school]);
    });
});
